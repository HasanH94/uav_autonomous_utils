...
from std_msgs.msg import Bool, Header  # Added Header import
import math
import signal
import sys
...
class StateManager:
    ...
    def launch_file(self, package, launch_file, publish_topic=None, *args):
        ...
        try:
            ...
            # Corrected type and length checks
    ...
    def publish_goal(self, x, y, z, publish_topic="/move_base_simple/goal"):
        ...
        #     rospy.loginfo(f"[StateManager] Waiting for subscribers to connect to {publish_topic}...")
        #     rate.sleep()
        timeout_seconds = 5
...
class DroneStateMachine:
    ...
    def logGpsStart(self, event=None):
        ...
        # Assuming you want to pass the current goal point as arguments
        try:
            current_goal = self.goal_points[self.counter]
            self.manager.launch_file('local_planner', 'local_node.launch', "/move_base_simple/goal",
                                    *current_goal)
            ...
            rospy.logerr("No more goals available. Moving to home position and landing.")
            rospy.logerr("No more goals available. Moving to home position and landing.")

            # Launch your local planner again so it can receive the home goal.
            self.manager.launch_file(
                'local_planner',             # same as in logGpsStart()
                'local_node.launch',
                "/move_base_simple/goal",    # topic to which we will publish the home goal
                home_position.pose.position.x, home_position.pose.position.y, home_position.pose.position.z              # the home position
            )
            self.set_offboard_mode()

            rospy.loginfo("[DroneStateMachine] Launched local_node for going home.")
            rospy.sleep(2)  # Give it time to start up
            
            # Optionally wait until the drone is at home
            

            while self.calculate_distance(self.current_position, home_position.pose.position) > self.threshold:
                rospy.sleep(1)
            ...
            try:
                ...
                if resp.mode_sent:
                    ...
                    rospy.logwarn("Failed to set LAND mode.")
            except rospy.ServiceException as e:
                rospy.logerr(f"Service call failed while setting LAND mode: {e}")
            
            # Shutdown the node gracefully
            rospy.signal_shutdown("No more goals. Landing at home.")
    ...
    def set_land_mode(self):
        ...
        mode = 'AUTO.LAND'  # Ensure that this mode is supported by your flight controller
        set_mode = SetModeRequest()
        set_mode.custom_mode = mode
        set_mode.base_mode = 0  # Typically set to 0 when specifying custom_mode
...
if __name__ == '__main__':
    ...
    # Register the shutdown handler with rospy.on_shutdown
    rospy.on_shutdown(sm.shutdown_handler)
    
    rospy.spin()

#can u tell me what will happen if I run this code? assume that each time the drone can not locate the object we want. 