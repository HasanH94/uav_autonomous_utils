    ...
    def callback_function(self, image_msg):
        ...
        if marker_IDs is not None and len(marker_IDs) > 0:
            ...
            for i in range(len(marker_IDs)):
                ...
                k_y = 0.0
                k_z = 0.0
                k_yaw = 0.0 # Gain for yaw control (tune based on your system)
                ...
                # self.body_x_vel = k_x * distance_z + kd_x * derivative_z + (integral_gain_x * self.integral_error_z)
                # self.body_y_vel = k_y * error_x_meters + kd_y * derivative_x + (integral_gain_y * self.integral_error_x)
                # self.body_z_vel = k_z * error_y_meters + kd_z * derivative_y + + (integral_gain_z * self.integral_error_y)
                # self.body_yaw_vel = k_yaw * yaw_error + kd_yaw * derivative_yaw + (integral_gain_yaw * self.integral_error_yaw)

                # global_vel_x = self.body_x_vel * math.cos(self.yaw) - self.body_y_vel * math.sin(self.yaw)
                # global_vel_y = self.body_x_vel * math.sin(self.yaw) + self.body_y_vel * math.cos(self.yaw)
                # global_vel_z = self.body_z_vel
                # global_vel_yaw = self.body_yaw_vel
                ...
                self.pose.linear.y = global_vel_y
                self.pose.linear.z = global_vel_z
                self.pose.angular.z = global_vel_yaw
