...
class StateManager:
    ...
    def transition_to_state(self, state_name):
        if state_name == 'visual_servoing':
            self.run_node('othmanPack', 'PID.py')
...
class DroneStateMachine:
    ...
    def __init__(self):
        ...
        self.machine.add_transition(trigger='object_lost', source='visual_servoing', dest='search_for_object', after='start_search_with_timer')
        ...
        self.machine.add_transition(trigger='timeout', source='search_for_object', dest='gps_navigation', after='start_gps')
    ...
    def task_done_callback(self, msg):
        if msg.data:  # If task_done is True
            if self.state == 'performing_task':
                self.task_done()
    ...
    def start_search_with_timer(self):
        ...
        self.search_timer = rospy.Timer(rospy.Duration(30), self.timeout_callback, oneshot=True)

    def timeout_callback(self, event):
        rospy.logwarn("Timeout reached. Switching back to GPS navigation.")
        self.timeout()
